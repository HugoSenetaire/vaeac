...
class Pad(nn.Module):
    ...
    def forward(self, input):
        return input[:,:,self.padding:-self.padding, self.padding:-self.padding]
...
prior_network = nn.Sequential(
    ...
    nn.Conv2d(6, 8, 1),
    ResBlock(8, 8), ResBlock(8, 8), ResBlock(8, 8), ResBlock(8, 8),
    MemoryLayer('#1'), # 224 x 224
    nn.AvgPool2d(2, 2), # 112 x 112
    ResBlock(8, 8), ResBlock(8, 8), ResBlock(8, 8), ResBlock(8, 8),
    MemoryLayer('#2'), # 112 x 112
    nn.AvgPool2d(2, 2), nn.Conv2d(8, 16, 1), # 56 x 56
    ResBlock(16, 8), ResBlock(16, 8), ResBlock(16, 8), ResBlock(16, 8),
    MemoryLayer('#3'), # 56 x 56
    nn.AvgPool2d(2, 2), nn.Conv2d(16, 32, 1), # 28 x 28
    ResBlock(32, 16), ResBlock(32, 16), ResBlock(32, 16), ResBlock(32, 16),
    MemoryLayer('#4'), # 28 x 28
    nn.AvgPool2d(2, 2), nn.Conv2d(32, 64, 1, 1), # 16 x 16
    ResBlock(64, 32), ResBlock(64, 32), ResBlock(64, 32), ResBlock(64, 32),
    MemoryLayer('#5'), # 16 x 16
    ...
    ResBlock(256, 128), ResBlock(256, 128),
    MemoryLayer('#7'), # 4x4
    nn.AvgPool2d(2, 2), nn.Conv2d(128, 256, 1), # 2x2
    ResBlock(256, 128), ResBlock(256, 128),
    ResBlock(256, 128), ResBlock(256, 128),
    MemoryLayer('#8'), # 2 x 2
    nn.AvgPool2d(2, 2), nn.Conv2d(256, 512, 1), # 1 x 1
...
generative_network = nn.Sequential(
    ...
    nn.Conv2d(256, 128, 1), nn.Upsample(scale_factor=2), # 2 x 2
    MemoryLayer('#8', True), nn.Conv2d(384, 128, 1),  # 2 x 2
    ResBlock(128, 64), ResBlock(128, 64),
    ResBlock(128, 64), ResBlock(128, 64),
    nn.Conv2d(128, 64, 1), nn.Upsample(scale_factor=2), # 4 x 4
    MemoryLayer('#7', True), nn.Conv2d(192, 64, 1), # 4 x 4
    ResBlock(64, 32), ResBlock(64, 32), ResBlock(64, 32), ResBlock(64, 32),
    nn.Conv2d(64, 32, 1), nn.Upsample(scale_factor=2), # 8 x 8
    MemoryLayer('#6', True), nn.Conv2d(96, 32, 1), # 8 x 8
    ResBlock(32, 16), ResBlock(32, 16), ResBlock(32, 16), ResBlock(32, 16),
    nn.Conv2d(32, 16, 1), nn.Upsample(scale_factor=2), Pad(padding=1), # 16 x 16
    MemoryLayer('#5', True), nn.Conv2d(48, 16, 1), # 14 x 14
    ResBlock(16, 8), ResBlock(16, 8), ResBlock(16, 8), ResBlock(16, 8),
    nn.Conv2d(16, 8, 1), nn.Upsample(scale_factor=2),
    MemoryLayer('#4', True), nn.Conv2d(24, 8, 1),
    ResBlock(8, 8), ResBlock(8, 8), ResBlock(8, 8), ResBlock(8, 8),
